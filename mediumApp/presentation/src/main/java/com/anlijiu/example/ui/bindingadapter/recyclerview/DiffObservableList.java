...
import com.anlijiu.example.utils.RxUtils;
    ...
    public void update(List<T> newItems) {
        ...
        Observable.create(new ObservableOnSubscribe<DiffUtil.DiffResult>() {
            ...
            public void subscribe(ObservableEmitter<DiffUtil.DiffResult> observableEmitter) throws Exception {
                DiffUtil.DiffResult result = doCalculateDiff(list, newItems);
                observableEmitter.onNext(result);
                observableEmitter.onComplete();
            }
        }).subscribe(result -> dispatchUpdates(newItems, result));
//        list = newItems;
//        diffResult.dispatchUpdatesTo(listCallback);
    }

    private void dispatchUpdates(List<T> newItems, DiffUtil.DiffResult result) {
        list = newItems;

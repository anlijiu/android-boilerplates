...
public final class DebugViewContainer implements ViewContainer {
    ...
    public ViewGroup forActivity(final Activity activity) {
        ...
        Timber.d("DebugViewContainer  forActivity %s", activity);
        final ViewHolder viewHolder = new ViewHolder();
        ButterKnife.bind(viewHolder, activity);

        final Context drawerContext = new ContextThemeWrapper(activity, R.style.Theme_M01_Debug);
...
//        }
        Timber.d("viewHolder.drawerLayout is %s", viewHolder.drawerLayout);
        viewHolder.drawerLayout.postDelayed(() -> {
            Timber.d("DebugViewContainer  postDelayed  start");
                viewHolder.drawerLayout.openDrawer(GravityCompat.END);
                Toast.makeText(drawerContext, R.string.debug_drawer_welcome, Toast.LENGTH_LONG).show();
            }, 2000);
        seenDebugDrawer.set(true);
        ...
        app.registerActivityLifecycleCallbacks(new ActivityLifecyclesServer.Empty() {
            @Override
